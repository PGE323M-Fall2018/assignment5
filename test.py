...
class TestSolution(unittest.TestCase):
	...
	def test_field_to_si_private(self):

	    conv = FieldtoSI()
	    assert abs(conv.viscosity(12) - 0.012) <= 1e-6
	    assert abs(conv.pressure(12) - 82737.12) <= 1e-6
	    assert abs(conv.permeability(7) - 6.9084631000000005e-15) <= 1e-6
	    assert abs(conv.volume(12) - 0.339802) <= 1e-6
	    assert abs(conv.volume_rate(110522) - 0.03622262174816) <= 1e-6
	    assert abs(conv.time(22) - 1900800) <= 1e-6
	    assert abs(conv.density(77)- 1.2334245000000001) <= 1e-6
	    assert abs(conv.compressibility(45.6) - 0.006613718243999791) <= 1e-6
	    assert abs(conv.temperature(66)- 18.88888888888889) <= 1e-6

	def test_si_to_field_private(self):
	    conv = SItoField()
	    assert abs(conv.viscosity(0.012) - 12) <= 1e-6
	    assert abs(conv.pressure(82737.12) - 12) <= 1e-6
	    assert abs(conv.permeability(6.9084631000000005e-15) - 7) <= 1e-6
	    assert abs(conv.volume(0.339802159104) - 12) <= 1e-6
	    assert abs(conv.volume_rate(0.03622262174816) - 110522) <= 1e-6
	    assert abs(conv.time(1900800) - 22) <= 1e-6
	    assert abs(conv.density(1.2334245000000001)- 77) <= 1e-6
	    assert abs(conv.compressibility(0.006613718243999791) - 45.6) <= 1e-6
	    assert abs(conv.temperature(18.88888888888889)- 66) <= 1e-6
